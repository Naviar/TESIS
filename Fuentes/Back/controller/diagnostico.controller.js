...
diagnosticoCtrl.crearDiagnostico = (req, res) => {
    ...
    db.query(query, function(err, data) {
        if (err) res.json({ error: err })
            ...
            db.query(`select id_formato_diagnostico from formato_diagnostico order by id_formato_diagnostico desc limit 0,1;`, (err, data) => {
                if (err) res.json(err);
                res.json(data);
            });
        }
...
diagnosticoCtrl.getEstudiantes = (req, res) => {
    ...
    db.query("SELECT e.*, u.nombre, u.apellido, u.correo, u.celular FROM estudiante AS e INNER JOIN usuario AS u ON e.usuario_id_usuario = u.id_usuario WHERE etapa = 1 AND u.fecha >= CURRENT_DATE - 1 YEAR", (err, data) => {
...
diagnosticoCtrl.getEtapas = (req, res) => {
    ...
    db.query("SELECT * FROM etapa", (err, data) => {
...
diagnosticoCtrl.getSectores = (req, res) => {
    ...
    db.query("SELECT * FROM sector", (err, data) => {
