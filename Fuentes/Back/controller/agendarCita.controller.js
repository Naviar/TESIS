...
agendarCitaCtrl.getHorarios = async(req, res) => {
    ...
    db.query(`SELECT * FROM horario WHERE tipo_reunion_id_tipo_reunion =${1}`, (err, data) => {
        if (err) {
            res.json({ error: err })
...
agendarCitaCtrl.getDisponibilidadId = async(req, res) => {
    ...
    db.query(`SELECT * FROM disponibilidad WHERE id_disponibilidad='${id}'`, (err, data) => {
        if (err) {
            res.json({ error: err })
            console.log("Hubo un error en la busqueda DE HORARIOS" + err);
        } else {
...
agendarCitaCtrl.disponibilidadesLibres = async(req, res) => {
    ...
    db.query(`SELECT * FROM horario WHERE id_horario='${id}' AND NOT EXISTS (SELECT id_disponibilidad FROM disponibilidad WHERE disponibilidad.horario_id_horario = horario.id_horario AND disponibilidad.fecha = '${fecha}');`, (err, data) => {
        if (err) {
            res.json({ error: err })
            console.log("Hubo un error en la busqueda DE HORARIOS" + err);
        } else {
...
agendarCitaCtrl.disponibilidadesOcupadas = async(req, res) => {
    ...
    db.query(`SELECT * FROM horario WHERE id_horario='${id}' AND EXISTS (SELECT id_disponibilidad FROM disponibilidad WHERE disponibilidad.horario_id_horario = horario.id_horario AND disponibilidad.fecha = '${fecha}');`, (err, data) => {
        if (err) {
            res.json({ error: err })
            console.log("Hubo un error en la busqueda DE HORARIOS" + err);
        } else {
...
agendarCitaCtrl.getAsesorias = async(req, res) => {
        ...
        if (err) {
            ...
            db.query(query2, (err, data2) => {
                ...
                if (err) { res.json({ error: true, msg: err }) } else {
                    ...
                    db.query(`SELECT * FROM horario WHERE tipo_reunion_id_tipo_reunion =${2} AND tipo_asesoria_id_tipo_asesoria = ${data2[0]['TIPO_ASESORIA_ID_TIPO_ASESORIA']}`, (err, data) => {
                        if (err) {
                            res.json({ error: err })
                            console.log("Hubo un error en la busqueda DE HORARIOS" + err);
                        } else {
            ...
            db.query(`SELECT * FROM horario WHERE tipo_reunion_id_tipo_reunion =${2} AND tipo_asesoria_id_tipo_asesoria = ${data[0][2]}`, (err, data) => {
                if (err) {
                    res.json({ error: err })
                    console.log("Hubo un error en la busqueda DE HORARIOS" + err);
                } else {
    ...
    //     db.query(`SELECT * FROM horario WHERE tipo_reunion_id_tipo_reunion =${2}`, (err, data) => {
    //         if (err) {
    //             res.json({ error: err })
    //             console.log("Hubo un error en la busqueda DE HORARIOS" + err);
    //         } else {
