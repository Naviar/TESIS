...
ConvocatoriaCtrl.getAnnouncements = (req, res) => {
    ...
    const query = `SELECT C.NOMBRE AS NOMBRE_CONVOCATORIA,C.ID_CONVOCATORIA, C.FECHA_INICIO, C.FECHA_FIN, C.FECHA_INFORME_INICIAL, C.FECHA_INFORME_FINAL, U.ID_USUARIO , U.NOMBRE , U.APELLIDO FROM CONVOCATORIAS AS C INNER JOIN USUARIO AS U ON C.ID_USUARIO = U.ID_USUARIO ORDER BY C.FECHA_INICIO ASC`;
