...
asesoriaCtrl.getEstudiantes = (req, res) => {
    ...
    db.query("SELECT e.*, u.nombre, u.apellido, u.correo, u.celular FROM estudiante AS e INNER JOIN usuario AS u ON e.usuario_id_usuario = u.id_usuario WHERE e.etapa = '4' AND u.fecha >= CURRENT_DATE - 1 YEAR;", (err, data) => {
...
asesoriaCtrl.postFormAsesoria = (req, res) => {
    ...
    db.query(query, (err, data) => {
        ...
        if (err) {
            ...
            db.query(`select id_formato_asesoria from formato_asesoria order by id_formato_asesoria desc limit 0,1;`, (err, data) => {
                res.json(data);
            });
        }
...
asesoriaCtrl.postCompromisos = async(req, res) => {
    ...
    for (let i = 0; i < compromisos.length; i++) {
        ...
        db.query(`INSERT INTO compromiso (actividad, fecha, observacion, id_usuario, formato_asesoria_id_formato_asesoria) 
