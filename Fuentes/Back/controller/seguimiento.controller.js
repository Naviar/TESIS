...
seguimientoCtrl.getEstudianteAsesoria = (req, res) => {
    ...
    ibmdb.open(connStr, (err, conn) => {
        ...
        conn.query(`SELECT e.*, u.nombre, u.apellido, u.correo, u.celular FROM estudiante AS e INNER JOIN usuario AS u ON e.usuario_id_usuario = u.id_usuario and e.id_estudiante = (select estudiante_id_estudiante from estudiante_has_formato_asesoria where formato_asesoria_id_formato_asesoria = '${id_asesoria}')`, (err, data) => {
...
seguimientoCtrl.getNameAsesoria = (req, res) => {
    ...
    ibmdb.open(connStr, (err, conn) => {
        ...
        conn.query(`SELECT NOMBRE_TIPO_ASESORIA FROM TIPO_ASESORIA WHERE ID_TIPO_ASESORIA = '${id_asesoria}'`, (err, data) => {
...
seguimientoCtrl.getCompromisosAsesoria = (req, res) => {
    ...
    ibmdb.open(connStr, (err, conn) => {
        ...
        conn.query(`SELECT c.actividad ,c.fecha, c.observacion , u.nombre , u.apellido, c.id_compromiso , u.rol_id_rol FROM COMPROMISO AS c INNER JOIN usuario AS u ON c.id_usuario = u.id_usuario WHERE c.formato_asesoria_id_formato_asesoria = '${id_asesoria}'`, (err, data) => {
